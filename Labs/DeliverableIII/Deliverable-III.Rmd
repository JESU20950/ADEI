...
=======
>>>>>>> a02eb430863f78229c4b7b5ce3c3bca0e4ae59be
...
<<<<<<< HEAD
###FALTAN EFECTES NETS I BRUTS  
=======


>>>>>>> d1603d6e2810c0dd4d67da0451e11db64ae67e69
...
m9a<-lm(Total_amount ~  f.tlenkm*(Fare_amount + f.espeed + Tip_amount + Tolls_amount),data=df)
summary(m9a)
m9aa<-step( m9a, k=log(nrow(df)))

m9b<-lm(Total_amount ~  f.tlenkm*f.espeed + Fare_amount + Tip_amount + Tolls_amount,data=df)
summary(m9b)
m9ba<-step( m9b, k=log(nrow(df)))

m9c<-lm(Total_amount ~  Fare_amount*(f.tlenkm + f.espeed + Tip_amount + Tolls_amount),data=df)
summary(m9c)
m9ca<-step( m9c, k=log(nrow(df)))
summary(m9ca)

m9d<-lm(Total_amount ~  Tip_amount*( Fare_amount+f.tlenkm + f.espeed+ Tolls_amount),data=df)
summary(m9d)
m9da<-step( m9d, k=log(nrow(df)))

m9e<-lm(Total_amount ~  Tolls_amount*( Fare_amount+f.tlenkm + f.espeed + Tip_amount),data=df)
summary(m9e)
m9ea<-step( m9e, k=log(nrow(df)))


BIC(m6,m9,m9a,m9b,m9c,m9ca,m9d,m9da,m9e,m9ea)
...
ll1<-Boxplot(rstudent(m9ca));ll1
sel2<-which(hatvalues(m9ca)>5*length(m9ca$coefficients)/nrow(df));sel2;length(sel2)
ll2<-which(row.names(df) %in% names(hatvalues(m9ca)[sel2]));ll2
sel3<-which(cooks.distance(m9ca)> 0.5 );sel3;length(sel3)
ll3<-which(row.names(df) %in% names(cooks.distance(m9ca)[sel3]));ll3

```
```
Para analizar las interacciones que pueden hacer mejorar nuestro modelo, consideramos el modelo m9 al disponer de dos variables factor necesarias por las especificaciones del report.

Después de aplicar una serie de interacciones entre factores así como entre numéricas y factores, vemos como aparecen 3 modelos con un BIC inferior a nuestro modelo sin interacciones m9, los cuales serían, en orden de mejor a peor, el m9ca,m9da y m9ea.

Aún así la mejora que obtenemos al aplicar estas interacciones no es lo suficientemente significativa como para justificar el aumento de complejidad  del modelo, así que como modelo principal seguiremos usando el m6.

Aquí podemos ver una comparación de las observaciones inusuales tanto de nuestro modelo como del que hemos escogido con interacciones(m9ca) y vemos como son bastante similares a excepción del individuo 322 que pasa a ser mucho más influyente en el modelo con interacciones. Aún así no hay una gran diferencia entre los demás individuos influyentes como para considerarlo mejor o peor por ello.
