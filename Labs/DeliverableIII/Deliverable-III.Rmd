...
# Interactiones
```{r}
m9a<- lm(Total_amount ~  (tlenkm +Fare_amount + espeed + Tip_amount + Tolls_amount) + (Trip_type +RateCodeID)^2 ,data=df) # Interaccions dobles en factors
m9a<-step(m9a, k=log(nrow(df)))
m9b<- lm(Total_amount ~  (tlenkm +Fare_amount + espeed + Tip_amount + Tolls_amount) * (Trip_type +RateCodeID)  ,data=df) # Interaccions dobles en factor-numèrica
m9b<-step(m9b, k=log(nrow(df)))
...
ll1<-Boxplot(rstudent(m9b));ll1
sel2<-which(hatvalues(m9b)>5*length(m9b$coefficients)/nrow(df));sel2;length(sel2)
ll2<-which(row.names(df) %in% names(hatvalues(m9b)[sel2]));ll2
sel3<-which(cooks.distance(m9b)> 0.5 );sel3;length(sel3)
ll3<-which(row.names(df) %in% names(cooks.distance(m9b)[sel3]));ll3
